<?php 
function ZO_UPS_display_adminmenu() {
	global $wpdb;
	global $ZO_UPS_config; // Genotec Global Vars
	global $_ultimateprojectstatus;

	// variables for the field and option names 
	$formname = 'ZO_UPS_adm01';
	...
	$myoptionlist = ZO_UPS_create_optionlist($formname);
	// options form
	echo '<div id="icon-plugins" class="icon32"></br></div>'."<h2>" . __( $ZO_UPS_config['name_long']. ' Settings', $formname ) . "</h2>";
	echo '<div class="wrap">';
	...
	echo '<h2>Edit Projects</h2>';
	$query = "select * from ".$_ultimateprojectstatus->table." ORDER BY Name";
	echo $_ultimateprojectstatus->do_createTable($wpdb->get_results($query,"ARRAY_A"),$_ultimateprojectstatus->table,0,1);
	
	echo '<h2>Edit Projects Status</h2>';
	$query = "select * from ".$_ultimateprojectstatus->tablestatus." ORDER BY StatusName";
	echo $_ultimateprojectstatus->do_createTable($wpdb->get_results($query,"ARRAY_A"),$_ultimateprojectstatus->tablestatus,0,1);
...
*/
function ZO_UPS_register_settings() {
	global $ZO_UPS_config; 
	// Get all fiels which as to get stored
	$option_array = array_keys($ZO_UPS_config['options']);
	
	// and go trough them
	for($i = 0; $i < count($option_array); $i++) {
		$options_array = array_keys($ZO_UPS_config['options'][$option_array[$i]]);
...
function ZO_UPS_create_optionlist($formname) {
	global $ZO_UPS_config; // Genotec Global Vars
	...
	for($i = 0; $i < count($option_array); $i++) {
		...
		for($j = 0; $j < count($options_array); $j++) {
			...
			$label = $ZO_UPS_config['options'][$option_array[$i]][$options_array[$j]];
